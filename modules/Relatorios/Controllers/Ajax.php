...
use Lidere\Modules\Relatorios\Models\AlteracaoEstrutura as modelAlteracaoEstrutura;
...
class Ajax extends Controller
{
    ...
    public function enviaEmailAlteracaoEstrutura()
    {
        $return = new \stdClass();
        $post = $this->app->request()->post();

        if(!empty($post['registros_ids'])){
           
           $registros = explode('-', $post['registros_ids']);

           if(!empty($registros)){
                foreach ($registros as $reg) {
                    $row = modelAlteracaoEstrutura::find($reg);
                    $upd['enviar'] =  '1';
                    $updated = $row->update($upd);
                }
           }
        }

        $return->error = !empty($updated) ? false : true;
    ...
    public function retornaItens()
    {
        ...
        $return->items = Item::whereRaw($filtros)->orderBy('cod_item','asc')
                            ->get();
