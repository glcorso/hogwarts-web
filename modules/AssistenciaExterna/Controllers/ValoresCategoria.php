...
/**
 * Controller Valores por Categoria
 *
 * @category   Modules
 * @package    Lidere\Modules
 * @subpackage AssistenciaExterna\Controllers\ValoresCategoria
 * @author     Ramon Barros <ramon@lideresistemas.com.br>
 * @copyright  2019 Lidere Sistemas
 * @license    Copyright (c) 2019
 * @link       https://www.lideresistemas.com.br/license.md
 */
class ValoresCategoria extends Controller
{
    /**
     * Rota do modulo
     *
     * @var string
     */
    public $url = 'assistencia-externa/valores-categoria';
    ...
    public function form($id = 0)
    {
        ...
        Assets::add('/assets/js/valorCategoriaForm.js', 'AssistenciaExterna');
    ...
    public function add()
    {
        ...
        $rules = array(
            'cod_lista' => 'required|max:10|unique:'.$connection.'.tsdi_assistencia_lista_cat,cod_lista',
        ...
        if ($this->validate($rules, $this->app->service->input, $messages)) {
            if ($this->app->service->add()) {
                Core::insereLog(
                    $this->modulo['url'],
                    'Valor por Categoria '.$this->app->filtros['descricao'].
                ...
                $this->app->flash(
                    'success',
                    'Valor por Categoria <strong>'.$this->app->filtros['descricao'].'</strong>'.
                ...
                $this->app->flash(
                    'error',
                    'Não foi possível criar o valor por categoria <strong>'.
            ...
            Core::insereLog(
                $this->modulo['url'],
                'Não foi possível alterar o valor por categoria '.$descricao.
            ...
            $this->app->flash(
                'error',
                'Não foi possível alterar o valor por categoria <strong>'.
    ...
    public function edit()
    {
        ...
        $descricao = $this->app->service->input['descricao'];
        $rules = array(
            'cod_lista' => 'required|max:10|unique:'.$connection.'.tsdi_assistencia_lista_cat,cod_lista,'.$id,
            'descricao' => 'required|max:70',
            'sit' => 'required'
        );
        $messages = array(
        ...
        if ($this->validate($rules, $this->app->service->input, $messages)) {
            if ($this->app->service->edit()) {
                ...
                $this->app->flash(
                    'success',
                    'Valor por Categoria <strong>'.$this->app->filtros['descricao'].
    ...
    public function delete()
    {
        if ($this->app->service->delete()) {
            Core::insereLog(
                $this->modulo['url'],
                'Valor por Categoria removido com sucesso pelo usuário '.$this->usuario['id'].
                ' - '.$this->usuario['nome'].'.',
                $this->usuario['id'],
                $this->empresa['id']
            );
            ...
            $this->app->flash(
                'success',
                'Valor por Categoria removido com sucesso!'
            );
            $this->app->redirect('/'.$this->modulo['url']);
        } else {
            $this->app->flash(
                'error',
                'Não foi possível remover o valor por categoria!'
            );
            $this->app->redirect($this->data['voltar']);
        }
    }
