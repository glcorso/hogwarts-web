 ...
 * @subpackage AssistenciaExterna\Controllers\Agrupadores
 * @author     Ramon Barros <ramon@lideresistemas.com.br>
 * @copyright  2019 Lidere Sistemas
 * @license    Copyright (c) 2019
 * @link       https://www.lideresistemas.com.br/license.md
 */
class Agrupadores extends Controller
{
    /**
     * Rota do modulo
     *
     * @var string
     */
    public $url = 'assistencia-externa/agrupador-itens';
    ...
    {
        Assets::add('/assets/js/agrupadoresForm.js', 'AssistenciaExterna');
    ...
    public function add()
    {
        ...
        if ($this->validate($rules, $this->app->service->input, $messages)) {
            if ($this->app->service->add()) {
                Core::insereLog(
                    $this->modulo['url'],
                    'Agrupador '.$this->app->filtros['descricao'].
                ...
                $this->app->flash(
                    'success',
                    'Agrupador <strong>'.$this->app->filtros['descricao'].'</strong>'.
                ...
                $this->app->flash(
                    'error',
                    'Não foi possível criar o Agrupador <strong>'.
            ...
            Core::insereLog(
                $this->modulo['url'],
                'Não foi possível alterar o Agrupador '.$descricao.
            ...
            $this->app->flash(
                'error',
                'Não foi possível alterar o Agrupador <strong>'.
    ...
    public function edit()
    {
        ...
        if ($this->validate($rules, $this->app->service->input, $messages)) {
            if ($this->app->service->edit()) {
                ...
                $this->app->flash(
                    'success',
                    'Agrupador <strong>'.$this->app->filtros['descricao'].
    ...
    public function delete()
    {
        if ($this->app->service->delete()) {
            Core::insereLog(
                $this->modulo['url'],
                'Agrupador removido com sucesso pelo usuário '.$this->usuario['id'].
                ' - '.$this->usuario['nome'].'.',
                $this->usuario['id'],
                $this->empresa['id']
            );
            ...
            $this->app->flash(
                'success',
                'Agrupador removido com sucesso!'
            );
            $this->app->redirect('/'.$this->modulo['url']);
        } else {
            $this->app->flash(
                'error',
                'Não foi possível remover o Agrupador!'
            );
            $this->app->redirect($this->data['voltar']);
        }
    }
